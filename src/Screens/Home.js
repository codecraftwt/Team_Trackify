...
const Home = ({ navigation, route }) => {
  ...
  const logoutAndNavigateToLogin = async () => {
    try {
      ...
      const rootNav = parentNav ? parentNav.getParent() : null;
  ...
  const fetchProfileData = useCallback(async () => {
    try {
      ...
      // 💡 CRITICAL: Ensure loading state is turned off after fetch attempt
  ...
  // Dependency on loadActiveState ensures we use the latest function definition.
  ...
  if (isProfileLoading) {
    return (
      <View style={styles.loadingContainer}>
        {/* You can re-add the ActivityIndicator here if you prefer a standard loader */}
        <ActivityIndicator size="large" color="#4385F4" />
        <Text style={{ fontSize: wp(5), color: "#000" }}>Loading Profile...</Text>
      </View>
    );
  }
  ...
  if (!profileData) {
    return (
      <>
        <View style={styles.loadingContainer}>
          <Text style={{ fontSize: wp(5), color: "#000", textAlign: 'center', marginBottom: 20 }}>
            Profile loading failed. Please connect with the Back office team.
          </Text>

          {/* 💡 ADDED LOGOUT BUTTON (Uses the now-fixed function) */}
          <TouchableOpacity
            style={styles.logoutErrorButton}
            onPress={logoutAndNavigateToLogin}
          >
            <Text style={styles.logoutErrorButtonText}>Logout</Text>
          </TouchableOpacity>

        </View>

        {/* The FancyAlert remains here */}
        <FancyAlert
          visible={fancyAlertVisible}
          onClose={() => { setFancyAlertVisible(false); setFancyAlertButtons(undefined) }}
          title={fancyAlertTitle}
          message={fancyAlertMessage}
          type={fancyAlertType}
          buttons={fancyAlertButtons}
        />
      </>
    )
  }
...
const styles = StyleSheet.create({
  ...
  logoutErrorButton: {
    marginTop: 20,
    backgroundColor: '#C6303E', // A clear, exit/danger color
    paddingVertical: 10,
