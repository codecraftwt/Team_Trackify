...
export default function AadharDetails() {
  ...
  const [type, setType] = useState(""); // Will store Aadhar type id
  const [loading, setLoading] = useState(true);
  const [uploading, setUploading] = useState(false);
  const [decodedToken, setDecodedToken] = useState(null);

  // Fetch document types and auto-select Aadhar type
  useEffect(() => {
    const initialize = async () => {
      try {
        ...
        if (Array.isArray(data)) {
          const activeTypes = data.filter(d => d.isActive && !d.isDeleted);
          console.log("Active Document Types:", activeTypes);

          // Find Aadhar type (handle spelling variations)
          const aadharDoc = activeTypes.find(d =>
            (d.documentType1 || d.documentType || "").toLowerCase().includes("adhar")
          );

          if (aadharDoc) {
            setType(String(aadharDoc.documentTypeId));
            console.log("Selected Aadhar Type ID:", aadharDoc.documentTypeId);
          } else {
            console.warn("Aadhar Card document type not found in API response");
          }
        } else {
  ...
  const handleSave = async () => {
    if (!aadharNumber.trim()) return Alert.alert("Validation", "Please enter Aadhar Number");
    if (!/^\d{12}$/.test(aadharNumber)) return Alert.alert("Validation", "Please enter a valid 12-digit Aadhar Number");
    if (!type) return Alert.alert("Validation", "Aadhar document type not found");
    ...
    try {
      ...
      const companyIdFromToken = parseInt(decoded.CompanyId ?? "0", 10);
      const employeeId = parseInt(decoded.EmployeeId ?? decoded.employeeId ?? "0", 10);

      if (!userIdFromToken || !companyIdFromToken || !employeeId) throw new Error("Invalid token: Missing IDs");
      ...
      const payload = {
        ...
        isDeleted: false,
        isActive: true,
        createLoginId: userIdFromToken,
        createDate: new Date().toISOString(),
        updateLoginId: userIdFromToken,
    ...
    <View style={styles.container}>
      ...
      <View style={styles.pickerContainer}>
        {loading ? (
          ...
          >
            <Picker.Item label="Select Type" value="" color="#9ca3af" enabled={false} />
            {documentTypes.map((item) => (
              <Picker.Item key={item.documentTypeId} label={item.documentType1} value={String(item.documentTypeId)} color="#212529" />
            ))}
          </Picker>
        )}
      </View>
...
const styles = StyleSheet.create({
  ...
  input: { height: hp(6), borderRadius: 8, borderWidth: 1, borderColor: "#E5E7EB", backgroundColor: "#fff", paddingHorizontal: wp(2), marginBottom: hp(2), color: "#212529", fontSize: 14 },
  attachBox: { height: hp(6), borderRadius: 8, borderWidth: 1, borderColor: "#E5E7EB", backgroundColor: "#fff", paddingHorizontal: wp(2), justifyContent: "center", marginBottom: hp(3) },
