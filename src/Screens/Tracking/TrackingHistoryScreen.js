// ============================================================================================
// Old wroking code for total delivery station
...
import { Calendar } from 'react-native-calendars';
import * as TrackingService from '../../services/TrackingService';
import * as AdminService from '../../config/AdminService';
import CustomHeader from '../../Component/CustomHeader';
import { useAuth } from '../../config/auth-context';
...
const TrackingHistoryScreen = ({ route }) => {
  ...
  const load = useCallback(
    async (isRefresh = false) => {
      ...
      try {
        const res = await AdminService.getUserSessions(targetUserId, startDate);
        ...
        const hasMoreFromApi = Boolean(res.data?.pagination?.hasMore) && list.length >= limit;
