...
//       if (trackingRef.current) startForegroundTimers();
//       setLoading(false);
//       return;
//     }
...
const LocationTrackingScreen = () => {
  ...
  const stopTracking = useCallback(async () => {
    ...
    try {
      await syncNativeBufferedPointsForSession(sessionId);
      await stopNativeForegroundService();
      await TrackingService.endSessionOfflineFirst(sessionId);
      await AsyncStorage.removeItem(LAST_SESSION_KEY);
      await syncPendingLocations().catch(() => undefined);
      await loadSessionLocations(sessionId, true);
  ...
  }, [loadSessionLocations, navigation, showAlert, stopForegroundTimers]);
